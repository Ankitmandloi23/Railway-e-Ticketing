...
//in database routes collection : route(name)= from city
...
router.get('/railway/trains/:route', async (req, res) => {
    try {
     ...
     const route = await routeModel.findOne({'_id': req.params.route }) //route ,name
        const result = await trainModel.find({ route: route.name }) //route, name
       res.status(200).json(result)
    } catch (err) {
        res.status(500).json(err)
    }
